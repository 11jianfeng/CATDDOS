...
CONSTANT_MENU = {
    ...
    16: ("λ - 拉普拉斯极限", "天体力学的边界喵~", MathConstants.laplace_limit),
...
# ------------------ 知识小课堂 ------------------
def knowledge_classroom():
    """猫娘数学常数知识小课堂喵~"""
    print(color(f"\n=== 猫娘数学常数小课堂 === {CatgirlEmoji.EXCITED}", T.HEADER))
    
    lessons = [
        "数学常数是宇宙的语言喵~ 它们出现在各种自然现象中！",
        "π不仅仅和圆有关，它还出现在概率论、物理学中喵~",
        "e是自然增长的极限，描述着世界的变化规律喵~",
        "黄金比例φ被认为是最美的比例，存在于贝壳、花朵中喵！",
        "许多数学常数是否是有理数，至今仍是未解之谜喵！",
        "计算常数的高精度值，是计算机和数学的完美结合喵~",
    ]
    
    for i, lesson in enumerate(lessons, 1):
        print(color(f"{i}. {lesson}", T.OKCYAN))
    
    print(color(f"\n{CatgirlEmoji.PRAYING} 数学的世界无限广阔，让我们一起探索更多奥秘喵~", T.OKGREEN))
